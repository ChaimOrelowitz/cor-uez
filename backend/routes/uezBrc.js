...
router.post('/:id/request-check', async (req, res) => {
  try {
    ...
    await supabase.from('uez_applications').update({ brc_status: 'checking', brc_name_control: lookup.nameControl, brc_nj_tax_id: lookup.njTaxId, brc_last_error: null, updated_at: startedAt }).eq('id', application.id);
    ...
    const { data, error } = await supabase.from('uez_applications').update({ brc_status: 'lookup_error', brc_checked_at: checkedAt, brc_last_error: message, updated_at: checkedAt }).eq('id', application.id).select('*').single();
    if (error) throw error;
